...
def shakespeare_writer():
  ...
  hist = inputs
  for i in range(FLAGS.generation_length):
      outputs, _ = model(hist)
      char_id = sample_next_char_id(outputs)
      generated_chars.append(index2char[char_id.numpy()[0]])
      new_hist = torch.cat((hist[0], char_id), 0)
      hist = new_hist.view(1, -1)
  #####################################################################
