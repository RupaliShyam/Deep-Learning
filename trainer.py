...
flags.DEFINE_float('weight_decay', 0.005, 'Weight decay (L2 regularization).')
...
def analyze_model_kernels():
  checkpoint = torch.load('experiments/demo/{}_lr_0.001.wd_0.005/best_model.pt'.format(FLAGS.label_type))
  k = [0,3,6,8,10]
  for i in k:
    kernel = 'features.{}.weight'.format(i)
    kernel_name = '{}_layer{}'.format(FLAGS.label_type, i+1)
    print(kernel_name)
    print(checkpoint[kernel].shape)
    kernel_weight = checkpoint[kernel]
    visualize_kernels(kernel_name, kernel_weight)
